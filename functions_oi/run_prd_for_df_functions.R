...
run_prd_for_df <- function(df, ruleYear = 2024, data_name = NULL) {
  ...
  if (!is.null(data_name)) {
    write.csv(
      data2,
      file = paste0(current_directory, "/policy-rules-database/output/results_", data_name, ".csv"),
      row.names = FALSE
    )
  }
...
compute_fiscal_effect_df <- function(df_pre,
  ...
  # 1. Create post-earnings DF -------------------------------------------
  df_post <- df_pre %>%
    dplyr::mutate(income = income + avg_te)

  data_name_pre  <- if (!is.null(data_name)) paste0(data_name, "_pre") else NULL
  data_name_post <- if (!is.null(data_name)) paste0(data_name, "_post") else NULL

  # 2. Run PRD at pre & post earnings ------------------------------------
  #    Parallelize with future when N >= 10 (avoids overhead for simple mode).
  #    Workers are pre-initialized with PRD code/parameters (see app.R),
  #    so we only pass the data via explicit globals.
  if (N >= 10) {
    pre_future <- future::future({
      run_prd_for_df(df_pre, ruleYear, data_name_pre)
    }, globals = list(df_pre = df_pre, ruleYear = ruleYear, data_name_pre = data_name_pre),
       seed = TRUE)

    post_future <- future::future({
      run_prd_for_df(df_post, ruleYear, data_name_post)
    }, globals = list(df_post = df_post, ruleYear = ruleYear, data_name_post = data_name_post),
       seed = TRUE)

    pre_res  <- future::value(pre_future)
    post_res <- future::value(post_future)
  } else {
    # Sequential for small N (simple mode) — avoids serialization overhead
    pre_res <- run_prd_for_df(
      df        = df_pre,
      ruleYear  = ruleYear,
      data_name = data_name_pre
    )
    post_res <- run_prd_for_df(
      df        = df_post,
      ruleYear  = ruleYear,
      data_name = data_name_post
    )
  }
  ...
  long_raw <- dplyr::bind_rows(
    ...
    dplyr::mutate(
      ...
      component_label = dplyr::case_when(
        ...
        component == "cdctc_fed"        ~ "CDCTC",
        component == "cdctc_state"      ~ "CDCTC",
      ...
      component_label2 = dplyr::case_when(
        ...
        component == "cdctc_fed"        ~ "CDCTC (Fed)",
        component == "cdctc_state"      ~ "CDCTC (State)",
  ...
  delta_by_component <- long_raw %>%
    dplyr::group_by(
      ...
      stateAbbrev, state_share, federal_share
    ) %>%
    dplyr::summarise(
  ...
  # 9. Pre-computed summaries (eliminate repeated pivots in app.R) ---------

  # Long-form payments by component, period, gov_type
  payment_long <- payment_by_component_government %>%
    tidyr::pivot_longer(
      cols      = c(state_payment_total, federal_payment_total),
      names_to  = "gov_type",
      values_to = "dollar"
    ...
    dplyr::mutate(
      gov_type = ifelse(gov_type == "state_payment_total", "State", "Federal")
    )

  # Aggregated payments by period, gov_type, type

  summary_by_gov_type <- payment_long %>%
    dplyr::group_by(period, gov_type, type) %>%
    dplyr::summarise(dollar = sum(dollar, na.rm = TRUE), .groups = "drop")

  # Component-level gains (collapsed across counties/shares)
  component_gains_summary <- delta_by_component %>%
    dplyr::group_by(component_label, type) %>%
    dplyr::summarise(
  ...
  # 10. Return building blocks used by app.R --------------------------------
  list(
    n_participants          = N,
